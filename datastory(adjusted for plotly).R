...
jy_theme <- function() {
  theme(
    legend.position = "right", legend.title = element_text(family = "Impact", colour = "#ec8130", size = 10),
    legend.background = element_rect(fill = "#000000"),
    legend.key = element_rect(fill = "#000000", colour = "#000000"),
    legend.text = element_text(family = "Impact", colour = "#ffffff", size = 10),
    plot.background = element_rect(fill = "#000000", colour = "#000000"),
    panel.background = element_rect(fill = "#000000"),
    axis.text = element_text(colour = "#ffffff", face = "bold"),
    plot.title = element_text(colour = "#ec8130", face = "bold", size = 18, 
                              family = "Impact", hjust = 0, margin = margin(0, 0, 5, 0, "pt")),
    axis.title = element_text(colour = "#ec8130", face = "bold", size = 13, family = "Impact"),
    ...
    axis.ticks = element_line(colour = "#ec8130"),
    axis.line = element_line(color = "#ec8130", size=2)
  )
}
theme_set(jy_theme())
colour_palette <- c("#eccb06","#a9a9f9","maroon","red","orange","palegreen","green","lightblue","blue","purple","pink","white","green","lightblue","blue","purple","pink","pink","green","lightblue","blue","purple","pink")
colour_gradient <- c("#eccb06","#a9a9f9","blue") #Low to high conc
colour_bkg <- c("#eccb06","#a9a9f9") #contrasting w each other and w palette
    ...
    column(8,
      ...
      shinyjs::hidden(
        div(
          ...
          sidebarLayout(
            mainPanel(
              withSpinner( #https://www.rdocumentation.org/packages/shinycssloaders/versions/1.0.0/topics/withSpinner
                ui_element = plotlyOutput("graph_1"),
          ...
          div(
            #actionButton is auto type = default
            actionButton("back_1", 
                         label = "Huh?"),
            actionButtonStyled("continue_1", 
                               class = "next",
                               type = "primary",
                               label = "What now?")
          )
...
server <- function(input, output, session){
  ...
  lapply(
    ...
    FUN = function(i){
      observeEvent(
        input[[paste("continue_", i, sep="")]], 
        {
          shinyjs::hide(paste("screen_",i, sep=""))
          shinyjs::show(paste("screen_",i+1, sep=""))
        }
      )
      observeEvent(
        input[[paste("back_", i+1, sep="")]],
        {
          shinyjs::hide(paste("screen_",i+1, sep=""))
          shinyjs::show(paste("screen_",i, sep=""))
        }
      )
    }
  )
  ...
  observeEvent(
    input$return_5,
    {
      shinyjs::hide("screen_5")
      shinyjs::show("screen_1")
    }
  )
  ...
  output$graph_1 <- renderPlotly({
    g1 <- fear_cmpd_ranked() %>%
      ...
      labs(title = stringr::str_wrap(paste("Top",user_start_rank_1(),"to",user_end_rank_1(),"Most Frequently Emitted Compounds when in Fear")),
  ...
  output$graph_2a <- renderPlot({
    ms_data %>% 
      ...
      geom_hex() +
      labs(title = stringr::str_wrap(paste("Distribution of compound with m/z =",user_cmpd_2(),"across different scenes")),
  ...
  output$graph_2b <- renderPlot({
    ms_data %>% 
      ...
      labs(title = stringr::str_wrap(paste("Distribution of scenes")),
           y = "Type of Scene", 
           x = "Count") + 
      theme(axis.line = element_line(linewidth = 0.3, arrow = arrow(type='closed', length = unit(5,'pt'))),
  ...
  output$graph_3_yes <- renderPlot({
    ms_data %>% 
      ...
      labs(title = stringr::str_wrap(paste("Distribution of Top", 
                         user_top_cmpd_3(), 
                         "to", 
                         user_bottom_cmpd_3(), 
                         "Fear Compounds in", 
  ...
  output$graph_4 <- renderPlot({
    ms_data %>% 
      ...
      labs(title = stringr::str_wrap(paste("Distribution of compound with m/z =", 
                         user_cmpd_4(), 
                         "across duration of", 
                         user_movie_4())),
  ...
  output$graph_5a <- renderPlot({
    ms_data %>%
      ...
      geom_boxplot(color=colour_palette[1], fill=colour_palette[2]) +
      labs(x = "Concentration per Pax per Scene", 
           y = "Movie",
           title = stringr::str_wrap(paste("Distribution of compound with m/z =", user_cmpd_5()))) +
      theme(axis.line = element_line(linewidth = 0.3, arrow = arrow(type='closed', length = unit(5,'pt'))),
  ...
  output$graph_5b <- renderPlot({
    ms_data %>%
      ...
      geom_point(color = colour_palette[1]) + geom_line(color = colour_palette[2]) + 
